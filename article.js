...
function renderArticleImage(post, index, isHero = false) {
  const description = post.images?.[index - 1];
  const asset = post.imageAssets?.[index - 1];
  if (!asset) return description ? renderImagePlaceholder(description, index, isHero) : "";
  return `
    <figure class="article-image${isHero ? " article-image--hero" : ""}">
      <img src="${asset}" alt="${description}" ${isHero ? "" : 'loading="lazy"'}>
      <figcaption>${description}</figcaption>
    </figure>`;
}
  ...
  return {
    ...
    html: `
      <article class="article-wrap" style="--art-one:${one}; --art-two:${two}; --art-three:${three}">
        ...
        ${images[0] ? renderArticleImage(post, 1, true) : ""}
        <div class="article-layout">
          ...
          <div class="article-body">
            ...
            ${post.sections.map((section, index) => `
              ...
              ${index < 3 && images[index + 1] ? renderArticleImage(post, index + 2) : ""}`).join("")}
            ...
            <section class="article-cta" aria-label="Ask about this product">
              ...
              <h3>${post.cta}</h3>
              <a href="${post.productSlug ? `products/${post.productSlug}/index.html` : "https://marwarmade.com"}" ${post.productSlug ? "" : 'target="_blank" rel="noreferrer"'}>${post.productSlug ? "View this piece" : "Check price & availability"} <span>↗</span></a>
            </section>
            ${related.length ? `
